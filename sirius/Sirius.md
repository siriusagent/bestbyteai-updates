...
-->
# Sirius v0.1.2-alpha.002

Alpha.002 opens the 0.1.2 line for everyone. It ships callable sub-agent
runtime v2 with a contextual inspector, signed ABI-scoped engine updates,
native website Keychain fill and opt-in Site Tools, a forensic Share Feedback
workflow, and Goal / browser reliability work.

The briefly published alpha.001 could fail engine start on installed apps
("runtime-v2 authority store could not be initialized") and was withdrawn the
same day. Alpha.002 fixes that startup path and is the first 0.1.2 build
offered through auto-update.
...
- The iMessage channel client now consumes SiriusMsg `0.0.1-alpha.15` at
  ...
  own Messages access.
- Fixed the alpha.001 engine-start failure on installed apps. Runtime-v2
  startup no longer depends on ambient asyncio task visibility, which the
  embedded worker runtime cannot guarantee; its waits now use an explicit
  deadline. Workers log a diagnostic when the underlying condition is
  present, and engine-start failures now record a path-free cause instead
  of only a generic message.
- Sparkle build version is `158` (`CFBundleVersion`), the primary comparison
  key for auto-update. Apps on build `157` or earlier will be offered this
  release.
- The signed core-runtime feed is the first public ABI-1 schema-2 channel,
  engine version `2026.08.30.2`, built with app version `0.1.2-alpha.002`.
